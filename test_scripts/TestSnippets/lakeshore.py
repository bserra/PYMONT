...
class LakeShore33x(object):
    ...
    * PID?: Control loop PID values query
    ...
    def __init__(self, TCP_IP='192.168.5.1', TCP_PORT=7777, BUFFER_SIZE=16*1024):
    ...
    def temperature_probes(self):
        """"""
        results = dict()
        T_K = [i for i in (self._send_command('KRDG? 0')).split(',')]
        for idx, i in enumerate(T_K):
            results[self._send_command('INNAME? '+INPUT_NAMES[idx]).strip()]=i
        return results

    def power_heaters(self):
        """"""
#        results = dict()
        results = {'H1':self._send_command('HTR? 1'),
                   'H2':self._send_command('HTR? 2')}
        return results

    def setpoints(self):
        """"""
        results = {'SETP1':self._send_command('SETP? 1'),
                   'SETP2':self._send_command('SETP? 2')}
        return results        
    
    def dump_sensors(self):
        """"""
        return {**self.power_heaters(), 
