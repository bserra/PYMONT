...
if __name__ == "__main__":
    ...
    df['Time'] = pd.to_datetime(df.Time, format='%d-%m-%Y_%H-%M-%S')
    df2['Time'] = pd.to_datetime(df2.Time, format='%d-%m-%Y_%H-%M-%S')
    ...
    #                ax1b.plot(test_dates,df[i],'--',label=i)
                ...
                ax1.plot(df['Time'].astype('O'),df[i],label=i)
    
    
            if 'P00' in i:
                ax2.plot(df['Time'].astype('O'),df[i],label=i)
