...
class TPG26x(object):
    ...
    * RST: RS232 test
    ...
    def _send_command(self, command):
        ...
        elif response != self.ACK:
            message = 'Serial communication returned unknown response:\n{}'\
                ''.format(repr(response))
            raise IOError(message)
            return {self.inst_id+'Gauge 1':(float('nan'),'[mbar] Pressure Gauge 1'),
                    self.inst_id+'Gauge 2':(float('nan'),'[mbar] Pressure Gauge 2')}
...
#        status_code1 = int(reply.split(',')[0])
        value1 = str(reply.split(',')[1])
#        status_code2 = int(reply.split(',')[2])
        value2 = str(reply.split(',')[3])
#        return (value1, (status_code1, MEASUREMENT_STATUS[status_code1]),
#                value2, (status_code2, MEASUREMENT_STATUS[status_code2]))
        return {self.inst_id+'Gauge 1':[value1,'[mbar] Pressure Gauge 1'],
                self.inst_id+'Gauge 2':[value2,'[mbar] Pressure Gauge 2']}
...
class TPG262(TPG26x):
    ...
    def __init__(self, port='COM1', baudrate=9600,**kwargs):
        ...
        :type baudrate: int        
        """
        super(TPG262, self).__init__(port=port, baudrate=baudrate,**kwargs)
