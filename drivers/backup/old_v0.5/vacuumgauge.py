...
class TPG26x(object):
    ...
    * RST: RS232 test
    ...
    def __init__(self, port = 'COM1', baudrate = 9600,timeout = 5,id='',type=''):
    ...
    def _send_command(self, command):
        ...
        if response == self._cr_lf(self.NAK):
            ...
            raise IOError(message)
            return {self.inst_id+'Gauge 1':float('nan'),
                    self.inst_id+'Gauge 2':float('nan')}
...
#                value2, (status_code2, MEASUREMENT_STATUS[status_code2]))
        return {self.inst_id+'Gauge 1':value1,
                self.inst_id+'Gauge 2':value2}
