    ...
    * CD: Command Delete                    # TBI
    ...
    def __init__(self, 
        ...
        try:
            ...
            self.instrument.baud_rate = 1200
            self.connected = True
        except:
...
#            raise IOError(message)
    ...
    def status(self):
        ...
        for key in REQ_ST.keys():
            char_str = data[REQ_ST[key]['pos']].encode()
...
#        C, D
    ...
    def dump_sensors(self):
        ...
        variables = ["UPS","Batt","BattEstC","BattEstT","OutLoad"]
        comments = ["[int] UPS status int from 4 bits",
                    "[int] Battery status int from 4 bits",
                    "[%] Battery estimated charge",
                    "[min] Battery estimated time",
                    "[%] Output load"]
        
        results = {self.inst_id+keyw:[str(results[keyw]), comm] for keyw, comm in zip(variables, comments)}
