...
class CommumicationServer(object):
    ...
    def __init__(self):
        ...
        self.vg = vg.TPG262(id='001',type='P')
...
class MonitoringWidget(pg.GraphicsWindow):
    ...
    def __init__(self, parent=None, **kargs):
        ...
        # Add a plot
        self.p1 = self.addPlot(0,0,labels =  {'left':'Temperature [K]', 'bottom':'Date'})
        self.p1.addLegend()
        self.p2 = self.addPlot(1,0,labels =  {'left':'Pressure [mbar]', 'bottom':'Date'})
        self.p2.addLegend()
        ...
        for keyw in self.header.keys():
            ...
            if 'Gauge' in keyw:
                self.monitor_curves[keyw] = self.p2.plot(self.monitor_data[keyw])
            elif ('Input' in keyw) or ('SETP' in keyw):
                self.monitor_curves[keyw] = self.p1.plot(self.monitor_data[keyw])
            else:
                print (keyw)
                self.monitor_curves[keyw] = pg.PlotCurveItem(self.monitor_data[keyw], pen='r')
                self.monitor_curves[keyw] = self.p3.addItem(self.monitor_curves[keyw])
...
#        self.resize(2000,2000)
        ...
        self.l = QtWidgets.QSplitter(QtCore.Qt.Vertical,self.main_widget) #self.main_widget,
        self.dc = MonitoringWidget()
        self.l.addWidget(self.dc)

        #Reorganising layouts and widgets for v0.4
...
#        self.l.addWidget(self.split_top)
        self.l.addWidget(self.split_bottom)
        self.l.setFocus()
        self.setCentralWidget(self.l)

        self.statusBar().showMessage('CRISLER Monitoring Tool v0.2')
