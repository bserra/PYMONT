...
class CommunicationServer(object):
    ...
    def __init__(self,time_fmt):
        ...
        self.instruments = {'Temperature controller':[self.ls,self.ls2],
                            'Vacuum gauge controller':[self.vg],
                            'Multimeter':[self.mm1,self.mm2],
...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.LakeShoreSelect = QtWidgets.QPushButton("Lakeshore 1")
        self.LakeShoreSelect.connect(self.switch_instrument)
        ...
        self.MultiMeterSelect = QtWidgets.QPushButton("Multimeter 1")
...
#        self.l.setFocus()
    ...
    def sendMultiMeter(self, instrument = self.dc.comm.instruments['Multimeter'][0]):
        if (self.MultiMeterSend.text() != '') & (self.dc._STARTED==True):
            resp = self.dc.comm.mm1._send_command(self.MultiMeterSend.text())
            self.logtime = datetime.datetime.now().strftime(self.DATETIME_FMT)
