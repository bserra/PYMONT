...
class InstructionServer(CommServ):
    ...
    def inst_server(self, host='134.171.34.55', port=21128):
            server_address = (host, port)
            print (sys.stderr, 'starting up on %s port %s' % server_address)
            self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            print (sys.stderr, socket.SOL_SOCKET, socket.SO_REUSEADDR)
            ...
            while True:
                ...
                for i in data:
                    a = self.CS.instruments_tag[i[0]]._send_command(i[1])
    ...
    def myreceive(self):
        ...
        while bytes_recd < socket.MSGLEN:
            ...
            chunk = self.sock.recv(min(socket.MSGLEN - bytes_recd, 2048))
            if chunk == '':
                raise RuntimeError("socket connection broken")
            chunks.append(chunk)
