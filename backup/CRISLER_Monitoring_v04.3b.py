...
class InstructionServer():
    ...
    def __init__(self, CommServ, sock=None):
        if sock is None:
    ...
    def inst_server(self, host='134.171.36.18', port=4500):
            server_address = (host, port)
            print ('Instruction server %s:%s established' % server_address)
            self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
#            print (socket.SOL_SOCKET, socket.SO_REUSEADDR)
            self.socket.bind(server_address)        
            print ('%s:%s waiting for client connection' % server_address)
            self.socket.listen(5)
            self._connected = True
            while True:
                print ('pouet')
                # Wait for a connection
                self.connexion, client_adress = self.socket.accept()
                print ('Connexion on ', self.connexion, client_adress)
#                data = connexion.recv(1024)
                data = self.myreceive()
                print ('Data received:', data)
                test = data
#                if not test: break
                if test.decode() == ':READ:ALL':
                    response = self.CS._read_setup()
                    self.connexion.send(json.dumps(response).encode())
                else:
                    if not 'MONITORING' in data.decode():
                        test = [i.split('|') for i in test.decode().split(';')]
                        for i in test:
                            response = self.CS.instruments_tag[i[0]]._send_command(i[1])
                            print ('> Data send:', response.encode())
                            self.connexion.send(response.encode())
    
                    else:
                        print (test, 'Not a valid instruction')
                        continue
                continue
    ...
    def myreceive(self):
        ...
        bytes_recd = 0
        socket.MSGLEN = 1000
        a = self.connexion.recv(1024)
        print (a)
        return a
#        while bytes_recd < socket.MSGLEN:
#            chunk = self.connexion.recv(min(socket.MSGLEN - bytes_recd, 2048))
#            if chunk == '':
#                raise RuntimeError("socket connection broken")
#            chunks.append(chunk)
...
class CommunicationServer(object):
    ...
    def __init__(self,time_fmt):
        ...
        # Lakeshore bottom (8 outputs)
        self.ls = ls.LakeShore33x(TCP_IP='134.171.5.180',
                                  id='001',
                                  type='T')
        # Lakeshore top (8 output)
        self.ls2 = ls.LakeShore33x(TCP_IP='134.171.5.181',
                                   id='002',
                                   type='T')
        print ('Connection to Lakeshore')

        # Multimeter top (8 outputs)
        self.mm1 = rs.HMC8012(TCP_IP='134.171.5.183',
                              id='001',
                              type='MM',
                              mode='DC_V')
        # Multimeter bottom (4 output)
        self.mm2 = rs.HMC8012(TCP_IP='134.171.5.182',
                              id='002',
                              type='MM',
                              mode='DC_I')
        print ('Connection to Multimeters')

        # Rigol power supply
        self.psu1 = rig.DP831A(port='COM6',
                               id='001',
                               type='PSU')
        print ('Connection to Power Supply')

        # Pfeiffer controller
        self.vg = vg.TPG262(port='COM4',
                            id='001',
                            type='P')
        print ('Connection to Pfeiffer Controller')

        self.osc1 = agi.DSO5034A(TCP_IP='134.171.5.184',
                                 port=5025,
                                 id='001',
                                 type='OS')
        print ('Connection to Oscilloscope')


        self.instruments_tag = {'T001':self.ls,
                                'T002':self.ls2,
                                'P001':self.vg,
                                'PSU001':self.psu1,
                                'MM001':self.mm1,
        ...
        self.instruments = {'Temperature controller':[0,[self.ls,self.ls2]],
                            'Vacuum gauge controller':[0,[self.vg]],
                            'Multimeter':[0,[self.mm1,self.mm2]],
                            'Power Supply':[0,[self.psu1]],
                            'Oscilloscope':[0,[self.osc1]]}
        self.df = time_fmt
        self._connected = True
        print ('Connection initialization done.')
...
#        battery_status = {**self.ups.dump_sensors()}
    ...
    def stop(self):
        ...
        [i.close() for i in self.instruments_tag.values()]    
...
class MonitoringWidget(pg.GraphicsWindow):
    ...
    def __init__(self, time_fmt, parent=None, **kargs):
        ...
        for keyw in self.header.keys():
            if ('MM' not in keyw) and ('PSU' not in keyw) and ('OS' not in keyw):
...
#        self.l.setFocus()
        ...
        self.show()        
        self.InstructionServ = InstructionServer(self.dc.comm)
